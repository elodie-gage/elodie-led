...
bool inLedRange(int index) { return index >= 0 && index < NUM_LEDS; }
...
class Snowflake {
 ...
 public:
  ...
  Snowflake(uint16_t accuPos, uint16_t accuSpeed, CRGB color, Side side)
      : accuPos(accuPos), accuSpeed(accuSpeed), color(color), side(side) {}

  void moveStep() { accuPos += accuSpeed; }

  int pos() const { return accuPos / 100; }
  ...
  void draw(CRGB* leds) const {
    ...
    switch (side) {
      case Side::Left:
        ledIndex1 = sideLength - 1 - pos();
        ledIndex2 = ledIndex1 - 1;
        break;
      case Side::Right:
        ledIndex1 = sideLength + pos();
        ledIndex2 = ledIndex1 + 1;
        break;
    };
...
void Snowflakes2::render(CRGB* leds) {
  ...
  snowflakes.erase(std::remove_if(snowflakes.begin(), snowflakes.end(),
                                  [](Snowflake snowflake) {
                                    return snowflake.pos() >= sideLength;
                                  }),
                   snowflakes.end());
